...
from SetRepresentations.set_representation import Ellipsoid, Polytope, Cross, InftyNorm
from TrackingInvariants.tracking_invariant import TrackingInvariant, sampleSchedule, s2sDynamics
...
def main():
    ...
    errorLogger = Logger(err_log_path,
        "iter,ep0,esx0,esz0,ecx0,ecz0,epd0,esxd0,eszd0,ecxd0,eczd0,um1,z00,z10,u1,epF,esxF,eszF,ecxF,eczF,epdF,esxdF,eszdF,ecxdF,eczdF\n"
    )
    setLogger = Logger(err_log_path,
    ...
    tracking_inv = TrackingInvariant(
        errorSet, romSet, sampleSchedule, lambda x0, rom0: s2sDynamics(mjInt, hlip, x0, rom0), errorLogger=errorLogger, setLogger=setLogger
    )

    converged = False
    while not converged:
        while tracking_inv.getProportionInSet() < 1:
            tracking_inv.iterateSetMap(verbose=True)
        tracking_inv.iterateSetMap(verbose=True, converged=True)
        if tracking_inv.getProportionInSet() == 1:
            converged = True
